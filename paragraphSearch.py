...
inputdata_caseIDs = '../data/casePar.IDs.v3.csv' # caseParIDs (paragraph-level IDs) and cases (caseNames)
inputdata_casePairs = '../data/casePar.pairs.v3.csv' # case pairs (at paragraph level)
tfidf_values = '../data/tfidf.values.csv' # tf-idf values for words in the case documents vocabulary, see tfidf.calculation_cases.v1.py

#### vectorization options (which algorithm to select for generating vectors from paragraphs) 
vec_algo = 'tfidf' # accepts: random, tfidf, bert, use, doc2vec (tf-idf vectorizer, BERT transformer, Universal Sentence Encoder, doc2vec from gensim)
bert_type = 'GermanBERT' # accepts: GermanBERT, DistilBERT, test (used, if vec_algo == bert)
tfidf_vec_type = 'module' # accepts: selfmade (self-coded), module (sklearn)
  # boosted NNB vs. NNB: search for 'boosted NNB' (lines 82-83)

##### parameters to vary during an experiment, to compare different semantic matching models at paragraph level
tfidf_mask_threshold = 2 # e.g. 2; above this value words will be kept (Extended Attention Mask), set word_minlen = 0 (if vec_algo == 'bert')
tfidf_vec_threshold = 2.45 # e.g. 2.42; above this value tokens will be used for vectorization for the tf-idf baseline vectorization (if vec_algo == 'tfidf')

##### other parameters, keep stable during comparison of algorithms
topresnum = 8 # how many of the top hits are considered in the calculation of summary results, i.e. %AP (recommended: 8)
...
data_limit = 9000 # how many paragraphs to load for testing (caution: memory problems may occur with the full dataset!), e.g. 9000
...
cos = torch.nn.CosineSimilarity(dim=-1, eps=1e-8) # how cosine similarity is calculated, using torch
casePair = defaultdict(lambda: [])
print("\n\n\nDATA: Loading case pairs")
...
for line in lines:
	...
	try:
		...
		if vec_algo == 'tfidf':
			...
			tokenlenmin = 5
		if re.search(r'^[a-z]{3}', token) and len(token) >= tokenlenmin: # use a token filter (boosted NNB)
		#if len(token) > 0:                                              # do not use a token filter (NNB)
...
def get_tfidf_vec(selsents):
	...
	if tfidf_vec_type == 'selfmade':
		...
		vocab_t = vector_vocab # vocabulary parameter used 
...
def get_use_vec(selsents):
	...
	i = 0
	for p in selsents: # go through the list of paragraphs to vectorize 
		p = p[:800] # algorithm-specific maximal length
...
print("\nSEARCH: Comparing query paragraph vectors with vectorzied search collection paragraphs:\n")
...
for q in queries:
	...
	for res in resultlist:
		...
		if caseID2 in allpairs:
			print("\n####PAIR###", caseID2, ": lawIDs:", lawIDs[caseID2], " ->", res[0][:200])
			pairhits += 1
